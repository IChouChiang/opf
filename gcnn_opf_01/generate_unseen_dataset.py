...
def precompute_topology_operators():
    ...
    for topo_id in range(N_TOPOLOGIES):
        print(f"  Computing operators for topology {topo_id} (Removed: {config.topology_pairs[topo_id]})...")
...
def worker_generate_chunk(
    ...
    while len(samples) < n_samples and attempts < max_attempts:
        ...
        # 3. Solve AC-OPF
        ppc_solve = {k: (v.copy() if isinstance(v, np.ndarray) else v) for k, v in ppc_topo.items()}
...
def main():
    ...
    operators, ppc_base, baseMVA, bus, gen, N_BUS, N_GEN = precompute_topology_operators()
    
    # 2. Prepare generator limits (shared)
    gen_bus_indices, PG_min, PG_max, QG_min, QG_max, gen_mask = extract_gen_limits(ppc_base)
    gen_limits = {
        "gen_bus_indices": gen_bus_indices,
        "PG_min": PG_min,
        "PG_max": PG_max,
