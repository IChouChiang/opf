...
def check_data():
    ...
    # 4. Check Unseen Data
    unseen_path = os.path.join(base_dir, "samples_unseen.npz")
    if os.path.exists(unseen_path):
        unseen = np.load(unseen_path)
        print(f"\n--- Unseen Data ---")
        print(f"Samples: {unseen['pd'].shape[0]}")
        print(f"Topology IDs: {np.unique(unseen['topo_id'])}")
    else:
        print(f"\n--- Unseen Data ---")
