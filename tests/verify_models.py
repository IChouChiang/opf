...
def test_dnn():
    ...
    assert output["v_bus"].shape == (batch_size, n_bus, 2), f"v_bus shape: {output['v_bus'].shape}"
    ...
    assert v_bus_shape in [(n_bus, 2), (1, n_bus, 2)], f"Single v_bus shape: {v_bus_shape}"
...
def test_gcnn():
    ...
    # Verify output shapes for single sample
    assert output_single["pg"].shape == (n_gen,), f"Single pg shape: {output_single['pg'].shape}"
    assert output_single["vg"].shape == (n_gen,), f"Single vg shape: {output_single['vg'].shape}"
    assert output_single["v_bus"].shape == (n_bus, 2), f"Single v_bus shape: {output_single['v_bus'].shape}"
